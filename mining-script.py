...
def mineJira(jira_repository, project, owner, manager, category, since_date, to_date):
...
def fillGitBugInfo(bugInfo, commit):            
    if(commit.merge):
...
def fetchBugFixInfoFromGit(git_repositories, jira_issues_keys, since_date, to_date):
    issues = {}
    offset = 0;
    interation = 1
...
def mineGit(git_repositories, project, since_date, to_date):
...
def filterTopFrequentWords(text):
    if(text is not None):
        text = text.lower()
...
def loadJiraBugFixDataset(project):
...
def loadGitBugFixDataset(project):
...
def runThirdStep(project_key, project_name):
...
def runSecondStep(git_repository, project, since_date, to_date):
    print("  [Step-2.0] Extracting bug-fix info of " + project + " from Git repository...")
    mineGit(git_repository, project, since_date, to_date)


def runFirstStep(jira_repository, project, owner, manager, category, since_date, to_date):
...
def mineBugFix(since_date, to_date):
...
def fetchBugChangeLog(jira, project, manager, category, issue_key):
    events = []
...
def mineBugsChangeLog():
    ...
    for index, row in projects.iterrows():
        ...
        with open("dataset/changelog/" + row['JiraName'].lower() +"-bug-fix-changelog-dataset.csv", 'a') as file:
...
def fetchBugCommentsLog(jira, project, manager, category, issue_key):
    events = []
...
def mineBugsCommentsLog():
    ...
    for index, row in projects.iterrows():
        log = pandas.DataFrame(columns=['Project', 'Manager', 'Category', 'Key', 'Author', 'CreationDate', 'Content'])
        offset = 0;
        count = 1;
        start_date = datetime.now()
        print(">Building a bug comments log for the " + row['Name'] + " project [" + start_date.strftime('%Y-%m-%d %H:%M:%S') + "]")
        print("  [Step-1.0] Loading CSV with bug-fix info...")
        dataset = loadBugFixDataset(row['JiraName'])
        ...
        for issue_key in issues_keys:
            if(issue_key is not None and isValidKey(issue_key)):
                offset += 1
                issue_timeline = fetchBugCommentsLog(jira, row['JiraName'], row['Manager'], row['Category'], issue_key)
                
                if(offset == 500):
                    print("  [Step-2.0." + str(count) + "] Comments log of " + str(offset) + " bug issues mined...")
                    count += 1
                    offset = 0;
        ...
        with open("dataset/comment-log/" + row['JiraName'].lower() +"-bug-fix-comment-log-dataset.csv", 'a') as file:
            log.to_csv(file, sep=';', encoding='utf-8', index=False)
...
def fetchBugCommitLog(project, manager, category, bug_key, commit):
    events = []
...
def mineBugsCommitsLog(since_date, to_date):
    last_repo = []
    ...
    for index, row in projects.iterrows():
        log = pandas.DataFrame(columns=['Project', 'Manager', 'Category', 'Key', 'CommitHash', 'IsMergeCommit',
                                        'Author', 'AuthorDate', 'Committer', 'CommitterDate', 'CommitMessageTopWords',
                                        'FileName', 'FilePath', 'ChangeType', 'IsSrcFile', 'IsTestFile', 
                                        'AddLines', 'DelLines', 'NoMethods', 'LoC', 'CyC', 'NoTokens'])
        tracked_commits = []
        interation = 1
            
        start_date = datetime.now()
        print(">Building a bug commits log for the " + row['Name'] + " project [" + start_date.strftime('%Y-%m-%d %H:%M:%S') + "]")
        print("  [Step-1.0] Loading CSV with bug-fix info...")
        dataset = loadBugFixDataset(row['JiraName'])
        ...
        for commit in RepositoryMining(path_to_repo=last_repo, since=since_date, to=to_date).traverse_commits():
            message = commit.msg.upper().strip()
            keys_in_message = extractKeys(message)
            ...
            for bug_key in bug_keys_list:
                if(bug_key in keys_in_message):
                    ...
                    commit_changes = fetchBugCommitLog(row['JiraName'], row['Manager'], row['Category'], bug_key, commit)
            ...
            if(len(set(tracked_commits)) == 500):
                print("  [Step 2.0." + str(interation) + "] Commits log of " + str(len(set(tracked_commits))) +  " commits performed...")
                interation += 1
                tracked_commits = []
        ...
        with open("dataset/commit-log/" + row['JiraName'].lower() +"-bug-fix-commit-log-dataset.csv", 'a') as file:
            log.to_csv(file, sep=';', encoding='utf-8', index=False)
